...
export default function IntentPage() {
  ...
  const { data: intent, loading, error } = usePublicResource(`intent:${intentSlug}`, () =>
    getIntentBySlug(intentSlug),
  );

  useEffect(() => {
    if (intent?.property?.id) {
      trackListingClick({ propertyId: intent.property.id, action: "view" });
    }
  }, [intent?.property?.id]);

  if (loading) return <LoadingState />;
  if (error || !intent) return <NotFoundState message="Page not found" />;
  ...
  return (
    <>
      <SmartPageMeta
        ...
        canonicalPath={`/resorts/intent/${intentSlug}`}
      />
      ...
      <Box sx={{ mx: "auto", maxWidth: 1024, px: { xs: 2, sm: 3 }, py: 3, pb: { xs: 12, sm: 3 } }}>
        ...
        <Box component="nav" sx={{ mb: 3, display: "flex", alignItems: "center", flexWrap: "wrap", gap: 0.75 }}>
          <Link component={RouterLink} to="/" underline="none" sx={crumbLinkSx}>
            Home
          </Link>
          <ChevronRightIcon sx={{ fontSize: 12, color: sp.muted }} />
          <Link component={RouterLink} to="/resorts" underline="none" sx={crumbLinkSx}>
            Resorts
          </Link>
          <ChevronRightIcon sx={{ fontSize: 12, color: sp.muted }} />
          <Link component={RouterLink} to={`/resorts/${property.slug}`} underline="none" sx={crumbLinkSx}>
            {property.name}
          </Link>
          <ChevronRightIcon sx={{ fontSize: 12, color: sp.muted }} />
        ...
        <Box sx={{ mt: 5, borderTop: `1px solid ${sp.divider}`, pt: 3, textAlign: "center" }}>
          <Link component={RouterLink} to={`/resorts/${property.slug}`} underline="hover" sx={{ fontSize: "0.875rem", color: sp.blue }}>
